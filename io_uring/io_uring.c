 ...
 */
 ...
 /*
 * Function: io_uring_try_cancel_requests
 * Description: Fungsi ini mencoba membatalkan permintaan (request) dalam IO ring. Fungsi ini dapat membatalkan semua permintaan atau hanya permintaan yang sesuai dengan kondisi tertentu (seperti permintaan yang terkait dengan thread SQPoll).
 * Parameters:
 *   - ctx (struct io_ring_ctx*): Pointer ke konteks IO ring yang berisi daftar permintaan.
 *   - tctx (struct io_uring_task*): Pointer ke struktur tugas yang terkait dengan permintaan.
 *   - cancel_all (bool): Flag untuk menentukan apakah semua permintaan harus dibatalkan.
 *   - is_sqpoll_thread (bool): Flag yang menunjukkan apakah permintaan terkait dengan thread SQPoll.
 * Returns:
 *   - bool: Mengembalikan true jika permintaan berhasil dibatalkan, false jika tidak.
 * Example usage:
 *   - io_uring_try_cancel_requests(ctx, tctx, true, false);
 */
...
/*
 * Function: io_match_linked
 * Description: Fungsi ini digunakan untuk memeriksa apakah ada permintaan yang sedang diproses dalam tautan permintaan (linked requests). Fungsi ini berguna untuk mengetahui status permintaan yang terhubung dan apakah ada permintaan yang masih dalam proses.
 * Parameters:
 *   - head (struct io_kiocb*): Pointer ke permintaan utama yang akan diperiksa apakah ada permintaan terkait yang masih diproses.
 * Returns:
 *   - bool: Mengembalikan true jika ada permintaan yang sedang diproses, false jika tidak.
 * Example usage:
...
static bool io_match_linked(struct io_kiocb *head)
{
	...
	/* Memeriksa apakah ada permintaan terkait yang sedang diproses */
...
/*
 * Function: io_match_task_safe
 * Description: Fungsi ini digunakan untuk mencocokkan permintaan dengan tugas tertentu dalam konteks yang aman terhadap kondisi race dengan timeout yang terhubung (linked timeouts). Fungsi ini digunakan untuk memeriksa apakah permintaan sesuai dengan kondisi pembatalan yang diinginkan (semua permintaan atau hanya permintaan untuk tugas tertentu).
 * Parameters:
 *   - head (struct io_kiocb*): Pointer ke permintaan yang akan diperiksa.
 *   - tctx (struct io_uring_task*): Pointer ke struktur tugas yang ingin dicocokkan.
 *   - cancel_all (bool): Flag untuk menentukan apakah semua permintaan harus dicocokkan atau hanya permintaan untuk tugas tertentu.
 * Returns:
 *   - bool: Mengembalikan true jika permintaan cocok, false jika tidak.
 * Example usage:
 *   - io_match_task_safe(head, tctx, true);
 */
...
/* Memeriksa apakah tugas yang diberikan cocok dengan permintaan */
if (tctx && head->tctx != tctx)
return false;
if (cancel_all)
return true;

/* Menghindari race condition dengan timeout yang terhubung */
